...
`AnimatedList` SHALL aceptar `duration`, `easing` (via CSS var), presets de entrada/salida (`enter`, `exit`), `stagger` (solo entradas), `as` (elemento root, default `div`) e `itemClassName`/`itemStyle` para el wrapper medible de cada hijo, además de `className` y `style` en el root — el root puede ser el propio grid/flex del consumer.
