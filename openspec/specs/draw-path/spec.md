# draw-path Specification

## Purpose
Componente `DrawPath`: wrapper genérico que envuelve un SVG provisto por el consumer y anima el dibujo de sus elementos con trazo (`path`, `line`, `polyline`, `circle`, `rect`, `ellipse`) mediante el motor SVG stroke, sin reestructurar el SVG original, customizable via props y CSS custom properties (`--aui-draw-*`), SSR-safe y respetuoso de `prefers-reduced-motion`.
