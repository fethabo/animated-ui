# click-spark Specification

## Purpose
Componente `ClickSpark`: variante **declarativa** del patrón one-shot imperativo del paquete. Envuelve sus `children` y emite una ráfaga breve de chispas en el punto de cada `pointerdown` dentro del contenedor, sin exponer ref ni handle imperativo. Las chispas se dibujan en un canvas overlay pasivo (`pointer-events: none`) recortado al contenedor; el RAF arranca en el primer click y se auto-detiene al vaciarse el pool. Es customizable via props, respeta `prefers-reduced-motion` y es SSR-safe.
