...
## Purpose
Componente `ScrollProgress`: barra fija que refleja el progreso de lectura de la página, calculado con lógica pura y escrito como CSS custom property directamente sobre el DOM via listener pasivo coalescido por RAF (sin re-renders de React), animada con `transform: scaleX` en el compositor. Customizable via props y CSS custom properties, accesible sin spam (decorativa, `aria-hidden`), activa bajo `prefers-reduced-motion`, SSR-safe y extensible.
