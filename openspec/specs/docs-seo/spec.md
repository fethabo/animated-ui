# docs-seo Specification

## Purpose
Metadatos de SEO de la web de documentación (`https://animated-ui-docs.fethabo.cloud`): title y meta description por vista e idioma, `link rel="canonical"` y alternates `hreflang` por ruta, metadatos base Open Graph/Twitter card en `docs/index.html`, y generación en build de `sitemap.xml`/`robots.txt` a partir del `registry.json`.
