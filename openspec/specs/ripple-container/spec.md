# ripple-container Specification

## Purpose
Componente `RippleContainer`: dibuja ondas circulares expansivas desde el punto exacto de cada click (material ripple), como nodos efímeros imperativos autolimpiados en `animationend` (sin estado React por onda, ondas concurrentes), en una capa `pointer-events: none` que no interfiere con la interacción de los children; customizable via props y `--aui-ripple-*`, SSR-safe y respetuoso de `prefers-reduced-motion` (fade estático sin expansión).
