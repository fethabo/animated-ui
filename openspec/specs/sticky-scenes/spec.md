# sticky-scenes Specification

## Purpose
Componente `StickyScenes`: ancla el viewport durante el scroll (inner wrapper `position: sticky`) y transiciona entre escenas declaradas con `StickyScenes.Scene`. El motor `subscribeScroll` calcula el progreso del scroll dentro del rango sticky y lo descompone en la escena activa (marcada con `data-aui-active`) y el progreso dentro de ella, exponiendo `--aui-scene-progress` y `--aui-scene-index` como CSS vars escritas directamente sobre el DOM sin React state en el hot path. Customizable via props y CSS custom properties, respeta `prefers-reduced-motion` manteniendo el scroll funcional, y es SSR-safe y extensible.
