# image-trail Specification

## Purpose
Componente `ImageTrail`: emite imágenes efímeras en el recorrido del puntero dentro de su contenedor, rotando secuencialmente el pool de `images`, con emisión por umbral de distancia y nodos autolimpiados en `animationend`. Precarga las imágenes tras el montaje, es customizable (tamaño, duración, cap de concurrencia, estilado de imágenes), respeta `prefers-reduced-motion` como no-op, y es SSR-safe y extensible, manteniendo los `children` interactivos.
