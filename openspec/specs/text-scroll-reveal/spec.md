# text-scroll-reveal Specification

## Purpose
Componente `TextScrollReveal`: párrafo particionado por palabra cuyas palabras se "encienden" progresivamente según el avance del scroll (highlight progresivo reversible), con una única CSS var de progreso escrita por el scroll-driver y opacidad/color por palabra resueltos con `calc()` por índice (cero JS por palabra por frame); tracking solo cerca del viewport, accesible (texto completo en `aria-label`, palabras `aria-hidden`), customizable via props y `--aui-text-scroll-*`, SSR-safe y respetuoso de `prefers-reduced-motion`.
