...
## Purpose
Componente `Marquee`: desplaza sus `children` en una cinta infinita sin costura implementada con CSS puro, con dirección/velocidad/pausa en hover configurables y un modo opcional acoplado a la velocidad de scroll, customizable via props y CSS custom properties (`--aui-marquee-*`), SSR-safe y respetuoso de `prefers-reduced-motion`.
