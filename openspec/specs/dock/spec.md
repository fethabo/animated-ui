...
## Purpose
Componente `Dock`: fila de ítems (`Dock.Item`) que se magnifican según la proximidad del cursor, con soporte de orientación horizontal/vertical, customizable via props y CSS custom properties (`--aui-dock-*`), totalmente interactivo por teclado, SSR-safe y respetuoso de `prefers-reduced-motion`.
