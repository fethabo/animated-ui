# fireworks-burst Specification

## Purpose
Componente `FireworksBurst`: overlay pasivo (`pointer-events: none`, sin RAF en reposo) que expone via ref un handle imperativo tipado (`FireworksBurstHandle`) con el método `fire(options?)` para disparar fuegos artificiales one-shot — uno o más cohetes ascienden desde la base del contenedor y explotan en partículas radiales con gravedad, drag y fade. El RAF arranca con el primer disparo y se auto-detiene cuando no quedan partículas vivas; disparos concurrentes comparten RAF y canvas. Es customizable via props (defaults) y opciones por disparo, usa aleatoriedad seedable, degrada a no-op de forma segura y es SSR-safe.
