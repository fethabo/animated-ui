# wavy-text Specification

## Purpose
Componente `WavyText`: ondula sus caracteres en loop continuo con CSS puro (split por carácter, `transform: translateY` con `animation-delay` escalonado), amplitud/velocidad/stagger configurables, accesible (texto completo en `aria-label`, caracteres `aria-hidden`), SSR-safe y respetuoso de `prefers-reduced-motion`.
