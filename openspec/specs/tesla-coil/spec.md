# tesla-coil Specification

## Purpose
Componente `TeslaCoil`: renderiza sobre `<canvas>` un nodo central desde el cual emanan rayos (arcos eléctricos jagged con glow) hacia afuera, regenerándose periódicamente. En hover dirige rayos más intensos hacia el cursor (con trigger configurable hover/click), tracking por ref sin re-renders por frame. Es customizable via props y CSS custom properties (`--aui-tesla-*`), respeta `prefers-reduced-motion` deteniendo la regeneración, y es SSR-safe, responsive y extensible.
