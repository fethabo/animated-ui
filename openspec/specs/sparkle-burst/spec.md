# sparkle-burst Specification

## Purpose
Componente `SparkleBurst`: overlay pasivo (`pointer-events: none`, sin RAF en reposo) que expone via ref un handle imperativo tipado (`SparkleBurstHandle`) con el método `fire(options?)` para emitir destellos one-shot — estrellas de 4 puntas de vida corta alrededor de un punto de origen, con envolvente de escala (crecen rápido y se encogen), rotación individual y fade, dibujadas por path en canvas (no glifos de fuente). Es customizable via props (defaults) y opciones por disparo, degrada a no-op de forma segura y es SSR-safe.
