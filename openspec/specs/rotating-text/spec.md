# rotating-text Specification

## Purpose
Componente `RotatingText`: rota cíclicamente entre una lista de palabras/frases con transición configurable (`fade` / `slide-up` / `flip`), layout estable entre palabras de largos distintos, accesible sin spamear lectores de pantalla, SSR-safe y respetuoso de `prefers-reduced-motion`.
