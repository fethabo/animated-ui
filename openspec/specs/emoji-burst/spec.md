# emoji-burst Specification

## Purpose
Componente `EmojiBurst`: overlay pasivo (`pointer-events: none`, sin RAF en reposo) que expone via ref un handle imperativo tipado (`EmojiBurstHandle`) con el método `fire(options?)` para disparar ráfagas de emojis (🎉 ❤️ 👍) con la física de abanico/gravedad/drag del patrón one-shot, renderizados con `fillText` sobre canvas sin ningún asset externo. Es customizable via props (defaults) y opciones por disparo, degrada a no-op de forma segura (antes de hidratación, sin canvas, o bajo `prefers-reduced-motion`) y es SSR-safe.
