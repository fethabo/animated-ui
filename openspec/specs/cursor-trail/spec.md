# cursor-trail Specification

## Purpose
Componente `CursorTrail`: monta un canvas overlay (`pointer-events: none`) que dibuja una estela que sigue al puntero dentro de su contenedor, en modos `particles` o `line`, con emisión throttleada por distancia y física en módulos puros testeables. Es customizable via props, respeta `prefers-reduced-motion` desactivándose por completo, y es SSR-safe y extensible, manteniendo los `children` interactivos.
