# use-in-view Specification

## Purpose
Hook `useInView`: reporta si un elemento referenciado interseca el viewport usando IntersectionObserver, con soporte de `threshold`, `rootMargin` y modo `once`, SSR-safe y con fallback visible cuando IntersectionObserver no está disponible. Es la base de visibilidad para componentes de scroll como `ScrollReveal`.
