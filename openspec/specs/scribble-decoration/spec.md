# scribble-decoration Specification

## Purpose
Componente `ScribbleDecoration`: renderiza garabatos decorativos hand-drawn (SVG procedural y seedable) que se dibujan con line-drawing al entrar al viewport o al montar, con una biblioteca de shapes builtin (`arrow`, `asterisk`, `spiral`, `underline`, `circle`) extensible por el consumer, customizable via props y CSS custom properties (`--aui-scribble-*`), SSR-safe y respetuosa de `prefers-reduced-motion`.
