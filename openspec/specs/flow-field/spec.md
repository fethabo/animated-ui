...
## Purpose
Componente `FlowField`: renderiza sobre `<canvas>` partículas que avanzan siguiendo un campo vectorial derivado de ruido coherente, dejando trazos orgánicos y persistentes. Es determinista por `seed`, customizable via props y CSS custom properties (`--aui-flow-*`), respeta `prefers-reduced-motion` con una composición estática pre-simulada, y es SSR-safe, responsive y extensible.
