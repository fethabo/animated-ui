# mouse-parallax Specification

## Purpose
Componente `MouseParallax`: desplaza capas declaradas con `MouseParallax.Layer` según la posición del mouse, escribiendo CSS custom properties directamente sobre el DOM (sin re-renders de React) y dejando que el desplazamiento y suavizado corran en CSS puro en el compositor. Customizable via props y CSS custom properties, SSR-safe, extensible y respetuoso de `prefers-reduced-motion`.
